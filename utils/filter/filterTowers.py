...
def formatTowerToString(tower: Tower, tiers: list[int], emotes: dict, category: str) -> str: 
    towerString = f"<:{tower.tower}:{emotes.get(tower.tower)}> "

    if tower.max != -1 and not tower.isHero:
        towerString += f"{tower.max}x "

    towerString += CATEGORIES[category].get(tower.tower, "")
...
def filterTowers(towers: List[Tower], emotes: dict) -> dict:
    towerKeys = {(category, tower) for category, tower in CATEGORIES.items() for tower in tower.keys()}
    ...
    for category, tower in towerKeys:
        ...
        if currentTower and currentTower.tower not in seenTowers:
            ...
            formattedTowerString = formatTowerToString(currentTower, tiers, emotes, category)
