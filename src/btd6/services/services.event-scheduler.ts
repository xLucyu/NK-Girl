...
export class EventScheduler {
  ...
  private async runEventChecks(): Promise<void> {
    ...
    for (const result of results) {
      ...
      await this.handleEventChange(event.eventType);
    }
  }
  ...
  private async runLeaderboardChecks(): Promise<void> {
    ...
    const results = await Promise.allSettled(
      Object.values(this.leaderboards).map(
        async leaderboard => {
          ...
          const payloads = await leaderboard.refresh(event as never);
  ...
  private async handleEventChange(
    ...
    console.log(
      `New ${eventType} event detected`
    );

    // Later:
    //
    // const cache = this.caches[eventType];
    //
    // upload event
    // announce event
  }
